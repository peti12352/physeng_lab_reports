...
\begin{figure}[H]
    ...
    \caption{Temperature-dependent resistance. Heating and cooling branches are plotted separately, highlighting the VO$_2$ insulator--metal transition near \SI{57}{\celsius}}
    \label{fig:task3_rt}
\end{figure}
...
We also attempted to calculate the resistance immediatly before the pulse and right after it, and compare. To do this, we measured the resistance using formula \ref{r_t} on the part of the signal at the readout level (where we can measure the resistance). This is shown in Fig. \ref{fig:resistance_part}. We see that immediatly after the pulse, the memrsitor switches to a low resistance state. We were expecting it to go back up to a high resistance state. However, in the evaluated time, it did not go back to a high resistance level. So we can not provide an upper limit for the characteristic time (only a lower bound, considering the size of the readout window after the pulse). We are not satisfied with this measurement, as we did not observe the switch back to the high resistance state. 

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.65\linewidth]{resistance_by_parts.png}
%     \caption{Resistance right before and right after the excitation pulse.}
%     \label{fig:resistance_part}
% \end{figure}
...
Future work could include detailed analysis of the relaxation dynamics to extract time constants, and investigation of oscillator circuits to demonstrate the device's potential for oscillating neural networks.
...
\section{Measurement Application}

We developed a \texttt{C\#} application with a GUI for the operation of the measurement devices and the data acquisition, logging and visualization in real time. We also did the data analysis using python.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{task2_screenshot.png}
 
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{task3_mid_measurement.png}

    \end{subfigure}
    \caption{Data acquisition application.}
    \label{fig:gui}
\end{figure}
