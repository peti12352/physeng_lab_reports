...
\begin{abstract}
We report the characterization of a Type-I spontaneous parametric down-conversion (SPDC) source and its associated coincidence detection electronics. Using a $\beta$-barium borate (BBO) crystal pumped by a \SI{405}{\nano\meter} laser, degenerate \SI{810}{\nano\meter} photon pairs are detected via ID100 single-photon avalanche diodes (SPADs). The SPDC emission cone is spatially mapped, yielding a half-opening angle $\phi_0 \approx 3^\circ$, consistent with the phase-matching condition. Coincidence timing reveals a \SI{4.75}{\nano\second} shift in the start-stop histogram. While partially attributed to a \SI{5}{\centi\meter} cable difference, kinematic analysis demonstrates that electronic processing delays account for the majority of this shift. The analog counter performance is rigorously evaluated, identifying a \SI{4}{\volt} saturation boundary dictated by the op-amp voltage drop, and a \SI{45.45}{\hertz} low-frequency limit resulting from the integrator RC time constant. Gated detection sweeps confirm that random coincidences scale precisely with the singles rates and temporal coincidence window, $c_{rand} = r_1 r_2 \Delta t$, fully capturing the stochastic background statistics of the measurement.
\end{abstract}

\section{Introduction}

In this experiment, we utilize Birefringent Phase Matching (BPM), specifically Type-I phase matching within a negative uniaxial $\beta$-barium borate (BBO) crystal. The high-frequency \SI{405}{\nano\meter} pump propagates as an extraordinary wave, while the degenerate \SI{810}{\nano\meter} down-converted photons propagate as ordinary waves. Because the ordinary refractive index is inherently angle-independent in a uniaxial crystal, this process geometrically guarantees a perfectly circular emission cone around the pump axis, with a half-opening angle strictly governed by the phase-matching kinematics (see Appendix A).
...
The marginal intensity clearly exhibits a peak at an apparent angular position of $\theta \approx 7.2^\circ$ on the goniometer. This value represents an arbitrary mechanical offset relative to the laboratory frame. Because the goniometer arms were not physically zero-calibrated to the pump axis, it is impossible to extract the absolute position of the pump beam or the absolute half-opening angle directly from this single-arm marginal data. Consequently, the value of $\phi_0 \approx 3^\circ$ is utilized strictly as a robust theoretical prior to interpret the relative geometry, derived directly from the exact phase-matching kinematics of our specific BBO crystal (Appendix A). 

The relative angular separation of approximately $6^\circ$ between the two symmetric coincidence peaks in the 2D map is consistent with the $2\phi_0 \approx 6^\circ$ expected from Type-I phase matching in BBO for degenerate 810 nm emission. The phase-matching condition, derived from momentum conservation, dictates $n_o(\omega_s) \omega_s \cos\theta_s + n_o(\omega_i) \omega_i \cos\theta_i = n_e(\omega_p, \theta_{pm}) \omega_p$. For degenerate down-conversion ($\omega_s = \omega_i = \omega_p/2$), symmetry mandates $\phi_0 = \theta_s = \theta_i \approx 3^\circ$. Tilting the crystal by $\pm 3^\circ$ directly alters the effective extraordinary refractive index $n_e$, shifting the phase-matching condition and displacing the cone peak. 

The full width at half maximum (FWHM) of the measured marginal peak was explicitly extracted to be $\Delta\phi = 2.00^\circ$. Because the goniometer sweep was conducted in discrete angular steps, this FWHM was determined using linear interpolation to find the angles where the count rate crossed the 50\% intensity baseline. This angular width is significantly broader than the geometric resolution limit of $0.21^\circ$. This broadening is physically driven by the finite waist of the pump beam within the crystal, which introduces a spread in the transverse momentum $\vec{k}_\perp$, relaxing the strict angular correlation.
...
\begin{figure}[H]
    ...
    \caption{Analog output voltage as a function of input pulse frequency plotted on a log-log scale, which linearizes the multi-decade sweep and shows the bounds at \SI{45.45}{\hertz} and \SI{4}{\volt}.}
    \label{fig:task7}
\end{figure}

The linear region defines the calibration factor for the counter. A linear fit of the data between \SI{100}{\hertz} and \SI{10}{\kilo\hertz} yields a calibration factor $K \approx \SI{0.221}{\milli\volt\per\hertz}$, accompanied by an empirical DC offset of \SI{9.09}{\milli\volt}. The system exhibits two nonlinear limits. The high-frequency upper limit is characterized by voltage saturation at approximately \SI{4}{\volt}. This is an electronic power rail limit, not an intrinsic frequency limit; the circuit operates on a \SI{5}{\volt} supply, and the operational amplifier suffers an approximate \SI{1}{\volt} internal dropout. 
...
Task 9 evaluated the system under an external sweep to confirm linear gated scaling (Figure~\ref{fig:task9}). To experimentally modulate the average count rate for these tests, one might classically assume that sweeping the frequency of a square wave to very low values would suffice. However, this method fundamentally fails: at low frequencies, the time between pulses exceeds the analog integrator's \SI{22}{\milli\second} RC time constant, causing the capacitor to discharge and severely corrupting the DC voltage reading. To work around this, the frequency was fixed at \SI{1}{\kilo\hertz} (safely above the \SI{45.45}{\hertz} RC cutoff), and the \textit{duty cycle} was swept instead. This modulated the average single-photon count rate while staying above the integrator's RC cutoff.
...
\section{Conclusion}
We have successfully characterized a Type-I BBO SPDC source. While the uncalibrated goniometer zero-point precluded an absolute geometric measurement of the emission cone relative to the pump, the observed spatial correlation peaks remain consistent with the $\phi_0 \approx 3^\circ$ half-opening angle predicted by Type-I phase-matching theory. Electronic profiling of the coincidence logic quantified the RC integrator limit (\SI{45.45}{\hertz}), op-amp saturation (\SI{4}{\volt}), and identified the \SI{4.75}{\nano\second} start-stop shift as an electronic, rather than optical, processing delay. Finally, the stochastic overlap model was verified to predict the random coincidence baseline accurately.

\appendix
\section{Phase-Matching Kinematics and Cone Geometry}

The geometry of the SPDC emission is strictly governed by momentum conservation ($\Delta\vec{k} = 0$). For degenerate Type-I down-conversion ($405\text{ nm} \rightarrow 810\text{ nm} + 810\text{ nm}$), the momentum vector triangle yields the scalar relationship:
\begin{equation}
    n_e(\omega_p, \theta) = n_o(\omega_s) \cos(\phi_{int})
\end{equation}
where $\phi_{int}$ is the internal half-opening angle of the emission cone relative to the pump axis inside the crystal. In a negative uniaxial crystal like BBO ($\beta$-BaB$_2$O$_4$), the pump propagates as an extraordinary wave with a refractive index $n_e(\omega_p, \theta)$ dependent on the propagation angle $\theta$ relative to the crystal optical axis. The daughter photons propagate as ordinary waves with an index $n_o(\omega_s)$ that is inherently angle-independent. This angle-independence physically guarantees that the transverse emission pattern is a perfectly circular ring \cite{guilbert}.

Using the empirical Sellmeier equations for BBO derived by Eimerl et al.\ \cite{eimerl}, the ordinary index for the signal and idler is $n_o(810\text{ nm}) \approx 1.6603$, while for the pump we have $n_o(405\text{ nm}) \approx 1.6926$ and $n_e(405\text{ nm}) \approx 1.5678$.

First, we solve for the \textit{collinear phase-matching limit} where the emission cone angle is zero ($\phi_{int} = 0^\circ \implies \cos(\phi_{int}) = 1$), which requires $n_e(\omega_p, \theta_{\text{collinear}}) = n_o(810\text{ nm}) = 1.6603$. Solving the index ellipsoid equation:
\begin{equation}
    \frac{1}{n_e(\theta_{\text{collinear}})^2} = \frac{\cos^2(\theta_{\text{collinear}})}{n_o(405\text{ nm})^2} + \frac{\sin^2(\theta_{\text{collinear}})}{n_e(405\text{ nm})^2} = \frac{1}{n_o(810\text{ nm})^2}
\end{equation}
yields a collinear phase-matching cut angle of $\theta_{\text{collinear}} \approx 29.16^\circ$.

To generate a spatially separated non-collinear emission pattern in the laboratory, commercial BBO crystals for Type-I SPDC are standardly cut slightly larger than this collinear limit, typically at $\theta_c \approx 29.2^\circ$ or $29.3^\circ$. At normal incidence to the crystal face ($\theta = 29.2^\circ$), BBO's negative uniaxial nature causes the pump extraordinary index to drop slightly:
\begin{equation}
    n_e(29.2^\circ) = \frac{1}{\sqrt{\frac{\cos^2(29.2^\circ)}{1.6926^2} + \frac{\sin^2(29.2^\circ)}{1.5678^2}}} \approx 1.6595
\end{equation}
Since $n_e(29.2^\circ) \approx 1.6595 < n_o(810\text{ nm}) \approx 1.6603$, collinear propagation is phase-mismatched, forcing the daughter photons to emit off-axis at an internal half-angle of:
\begin{equation}
    \cos(\phi_{int}) = \frac{1.6595}{1.6603} \approx 0.9995 \implies \phi_{int} \approx 1.72^\circ
\end{equation}
Upon exiting the crystal face, these daughter photons refract at the interface. Applying Snell's Law ($n_{\text{BBO}} \sin\phi_{int} = n_{\text{air}} \sin\phi_{ext}$) with $n_{\text{air}} \approx 1.0$:
\begin{equation}
    \phi_{ext} = \arcsin(1.6603 \times \sin(1.72^\circ)) \approx 2.86^\circ \approx 3^\circ
\end{equation}
Because the physical cut angle $\theta_c$ is fixed by the manufacturer, the experimenter utilizes a mechanical goniometer to tune the effective propagation angle $\theta$ relative to the pump beam. Under a $+0.1^\circ$ tilt ($\theta = 29.3^\circ$), the extraordinary index drops further to $1.6593$, opening the internal angle to $1.94^\circ$ and the external angle to $3.22^\circ$. This mechanical degree of freedom allows the experimenter to dial in the spatial overlap of the SPDC cones at the $\sim 3^\circ$ collection geometry in the laboratory.

This derivation outlines standard phase-matching kinematics for negative uniaxial crystals, following the theoretical frameworks established by Eimerl et al.\ \cite{eimerl} and Couteau \cite{couteau}. The resulting geometry directly explains the $3^\circ$ external opening angle, aligning with the practical BBO alignment and tuning procedures detailed by Guilbert et al.\ \cite{guilbert}.

\begin{thebibliography}{9}
\bibitem{guilbert} 
H. Guilbert, Y. Wong, and D. J. Gauthier, \textit{Observation of elliptical rings in Type-I spontaneous parametric down-conversion}, arXiv:1503.04676 (2015).
\bibitem{eimerl} 
D. Eimerl, L. Davis, S. Velsko, E. K. Graham, and A. Zalkin, \textit{Optical, mechanical, and thermal properties of barium borate}, Journal of Applied Physics 62, 1968 (1987).
\bibitem{couteau} 
C. Couteau, \textit{Spontaneous parametric down-conversion}, Contemporary Physics 59, 291 (2018), arXiv:1809.00127.
\end{thebibliography}
