...
\section{Conclusion}
We have successfully characterized a Type-I BBO SPDC source. While the uncalibrated goniometer zero-point precluded an absolute geometric measurement of the emission cone relative to the pump, the observed spatial correlation peaks remain fully consistent with the $\phi_0 \approx 3^\circ$ half-opening angle rigorously mandated by Type-I phase-matching theory. Rigorous electronic profiling of the coincidence logic quantified the RC integrator limit (\SI{45.45}{\hertz}), op-amp saturation (\SI{4}{\volt}), and isolated the \SI{4.75}{\nano\second} start-stop shift as an electronic, rather than optical, processing delay. Finally, the stochastic overlap model was verified to accurately predict the continuous random coincidence baseline.
